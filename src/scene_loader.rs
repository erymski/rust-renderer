use crate::geometry::{
    AmbientLight, C, Camera, DirectionalLight, P, Plane, RenderingSet, Scene, Sphere, UNIT_Y,
    UNIT_Z, V, colors,
};
...
pub(crate) fn build_scene2() -> Scene {
    ...
    // large red sphere
    scene.add_object(Sphere::new(P(-1.35, 1.0, 0.2), 1.0, C(0.75, 0.08, 0.06)));

    // small green sphere
    scene.add_object(Sphere::new(P(2.2, 0.35, 1.3), 0.35, C(0.20, 0.65, 0.08)));

    // gray sphere (metallic in future)
    scene.add_object(Sphere::new(P(1.25, 0.65, -0.1), 0.65, C(0.85, 0.88, 0.92)));
